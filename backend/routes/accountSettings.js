...
router.put("/profile", authenticateToken, async (req, res) => {
  console.log("in profile put", req.body);
  ...
  if (req.body.first_name) {
    ...
    if (req.body.new_email !== req.body.confirm_email) {
      return res.status(400).json({
        error: true,
        message: "new email and confirm email should be same",
      });
    }
    ...
    if (validPass) {
      ...
      try {
        ...
        // return res
        console.log(err).status(400).json({
          error: true,
          message:
            "We were unable to update the account. Please refresh the page and try again.",
        });
    ...
    try {
      ...
      const { _id, first_name, email: emailId } = userData;
      const user = { id: _id, first_name: first_name, email: emailId };
      const accessToken = jwt.sign(user, process.env.SECRET_KEY_TO_ACCESS);
      return res
        .status(200)
        .json({
...
router.post("/addAddress", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const prevAddress = user.address.find(
      (ele) =>
        ele.street + ele.city + ele.state + ele.zip + ele.phone + ele.name ===
    ...
    if (prevAddress) {
      ...
      return res
        .status(400)
        .json({ error: true, message: "Already the given address is present" });
    } else {
      user.address = [
        ...user.address,
        { street, city, state, phone, zip, name },
      ];
...
router.put("/editAddress", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const prevAddress = user.address.filter(
      (ele) =>
        ele.street + ele.city + ele.state + ele.zip + ele.phone + ele.name ===
        street + city + state + zip + phone + name
    );
...
router.post("/addCard", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const prevCards = user.credit_cards.find(
      (ele) =>
        ele.card_number +
          ele.expires_on +
          ele.security_code +
          ele.postal_code ===
        card_number + expires_on + security_code + postal_code
    );
    if (prevCards) {
      console.log(prevCards);
      return res
        .status(400)
        .json({ error: true, message: "Already the given card is present" });
    } else {
      user.credit_cards = [
        ...user.credit_cards,
        { card_number, expires_on, security_code, postal_code },
